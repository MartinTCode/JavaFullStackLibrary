...
public class ModifyJournalLibrarianController {
    ...
    private void clickedDeleteJournalButtonLibrarian(MouseEvent event) {
        if (itemToModify == null) {
            ...
            alert.setContentText("Cannot delete a journal that hasn't been saved yet.");
            alert.showAndWait();
            return;
        }
        ...
        if (confirmDialog.showAndWait().orElse(null) == javafx.scene.control.ButtonType.OK) {
            try {
                ...
                successAlert.setContentText("The journal \"" + itemToModify.getTitle() + 
                ...
                errorAlert.setContentText("An error occurred while deleting the journal: " + 
                                    e.getMessage());
